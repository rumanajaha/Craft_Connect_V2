...
export async function searchUsers(supabase, { q = '', type = 'all', currentUserId = null }) {
  ...
  if (searchType === 'creator' || searchType === 'all' || searchType === 'creators') {
    const { data: creators, error: creatorsError } = await supabase
      .from('CreatorProfile')
      .select('id, owner_user_id, display_name, niches, ai_tags, created_at, bio, avatar_url');
  ...
  const formattedCreators = creatorsFiltered.map(creator => {
    const avatarUrl = creator.avatar_url || avatarMap[creator.owner_user_id] || fallbackAvatar;
    return {
      id: creator.id,
