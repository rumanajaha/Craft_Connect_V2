...
export async function DELETE(request) {
  try {
    ...
    // 2. Call delete_customer_account Postgres RPC using supabaseAdmin (requires service role key)
    const { error: dbError } = await supabaseAdmin.rpc("delete_customer_account", {
    ...
    // 4. Best-effort clean up user files in customer-avatars storage bucket
    await deleteFolder("customer-avatars", user.id);
